// Shared HTTP client for all OpenWebNinja / RapidAPI calls.
...
export interface OpenWebNinjaCallOptions {
  ...
  apiKey: string;
  /** Override the host (useful for testing or custom endpoints) */
  hostOverride?: string;
}
...
/**
 * Execute a GET request to an OpenWebNinja RapidAPI endpoint.
 */
export async function callOpenWebNinja<T = Record<string, unknown>>(
  ...
  const host = options.hostOverride || definition?.rapidApiHost;
  if (!host) {
    return {
      ok: false,
      status: 0,
...
function extractErrorMessage(body: any, status: number): string {
  ...
  return `RapidAPI error: ${status}`;
}

/** Check if a RapidAPI error indicates invalid/missing key */
export function isCredentialError(result: OpenWebNinjaCallResult): boolean {
  return result.status === 401 || result.status === 403;
}

/** Check if a RapidAPI error indicates quota exceeded */
